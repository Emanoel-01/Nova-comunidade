...
import { CategoriaForum, ComunidadeStateService, ForumTopico } from './comunidade-state.service';
          ...
          <div class="space-y-4">
            ...
            <div class="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                [class]="novoTitulo().trim() && novoConteudo().trim()
          ...
          <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-7 shadow-xs hover:border-slate-300 transition-all space-y-4">
            ...
            <div class="space-y-2">
              ...
              <div class="flex items-center gap-2 text-xs text-slate-500">
                ...
                <span class="font-bold text-slate-800">{{ topico.autor }}</span>
                <span>•</span>
                <span>{{ topico.cargo }}</span>
              </div>
            </div>
            ...
            <div class="pt-3 border-t border-slate-100 flex items-center justify-between gap-3 flex-wrap">
              ...
              <div class="flex items-center gap-3">
                ...
                <button
                  type="button"
                  (click)="state.toggleCurtirTopico(topico.id)"
                  [class]="topico.curtido ? 'bg-rose-50 text-rose-600 border-rose-200' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100'"
                  class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-bold transition-all cursor-pointer"
                >
                  <svg
                    [class]="topico.curtido ? 'fill-rose-500 text-rose-500' : 'fill-none text-slate-500'"
                    class="w-4 h-4"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                      ...
                      <div class="p-4 bg-white rounded-xl border border-slate-200/80 shadow-2xs space-y-2">
                        <div class="flex items-center justify-between gap-2">
                          <div class="flex items-center gap-2">
                            ...
                            <div>
                              <span class="text-xs font-bold text-slate-900">{{ resp.autor }}</span>
                              <span class="text-[11px] text-slate-400 ml-1.5">• {{ resp.cargo }}</span>
                            </div>
                          </div>
                          <span class="text-[10px] text-slate-400">{{ resp.tempo }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-end pt-1">
                          <button
                            type="button"
                            (click)="state.toggleCurtirResposta(topico.id, resp.id)"
                            [class]="resp.curtido ? 'text-rose-600 font-bold' : 'text-slate-400 hover:text-slate-600'"
                ...
                <div class="pt-3 border-t border-slate-200/80 space-y-2">
                  ...
                  <div class="flex flex-col sm:flex-row items-stretch sm:items-end gap-2">
                    ...
                    <button
                      ...
                      class="px-5 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-xs transition-colors shrink-0 cursor-pointer"
                    >
                      Enviar Resposta
                    </button>
                  </div>
...
export class ComunidadeForumComponent {
  readonly state = inject(ComunidadeStateService);
  readonly topicos = this.state.topicosForum;
  ...
  enviarResposta(topicoId: string): void {
    const texto = this.getRespostaInput(topicoId).trim();
    if (!texto) return;

    this.state.adicionarRespostaTopico(topicoId, texto);
    this.respostaInputs.update(dict => ({
      ...dict,
      [topicoId]: ''
    }));
  ...
  publicarTopico(): void {
    ...
    this.state.adicionarTopico(t, this.novaCategoria(), c);
    this.fecharFormulario();
  }

  getCategoriaBadgeEstilo(cat: CategoriaForum): string {
    switch (cat) {
